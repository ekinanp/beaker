...
module Beaker
  ...
  class TestSuiteResult
    ...
    def summarize_times(summary_logger)
      timing_trees = @test_cases.map do |test_case|
        hash_tree = JSON.parse(test_case.timed_node.to_json)
        hash_tree["test_file"] = hash_tree.delete("name")
        cleanup_tree = lambda do |tree|
          tree["runtime"] = tree.delete("content")
          tree.delete("json_class")
          tree["children"].each { |child| cleanup_tree.call(child) } if tree["children"]
        end
        cleanup_tree.call(hash_tree)
        hash_tree
      end
      summary_logger.notify(JSON.pretty_generate(timing_trees))
    end
